...
router.post("/hint", (req, res) => {
  try {
    const { type, excludedCategories = [] } = req.body || {};
    const secret = getSecretElement();
    const elements = getElementsList() || [];
    ...
    if (type === "secret_info") {
      const allInfos = [
        {
          category: "type",
          text: `Its type includes: <strong>${Array.isArray(secret.type) ? secret.type[0] : secret.type}</strong>`
        },
        {
          category: "lieu",
          text: `It can be found in: <strong>${Array.isArray(secret.lieu) ? secret.lieu[0] : secret.lieu}</strong>`
        },
        {
          category: "couleur",
          text: `Its color includes: <strong>${Array.isArray(secret.couleur) ? secret.couleur[0] : secret.couleur}</strong>`
        },
        {
          category: "hitbox",
          text: `Its hitbox is: <strong>${Array.isArray(secret.hitbox) ? secret.hitbox[0] : secret.hitbox}</strong>`
        }
      ];

      const availableInfos = allInfos.filter(
        (info) => !excludedCategories.includes(info.category)
      );
      const poolToUse = availableInfos.length > 0 ? availableInfos : allInfos;

      const index = getSeededRandom(2) % poolToUse.length;
      return res.json({
        text: `<strong>Secret Info:</strong> ${poolToUse[index].text}`
      });
    }
